...
__launch_bounds__(BLOCK_SIZE) __global__ void flash_attention_permute_kernel(
...
)
{
	...
	const uint32_t C = NH * HS;
	const uint32_t inp_C = C * 3; // QKV are stored together hence the 3C
	const int b = blockIdx.x;
	const int nh = blockIdx.y;
	const int t = blockIdx.z * Br;

	// Permute from (B, T, NH, HS) to (B, NH, T, HS)
	const float* q = inp + (b * T * inp_C) + nh * HS;
	const float* k = q + C;
	const float* v = k + C;

	// move to the correct sequence for q only
	q += t * inp_C;
	...
	{
		auto q4 = reinterpret_cast<const float4*>(&q[(inner_row + offset) * inp_C + inner_col]);
	...
	{
		wmma_tf32_ldmatrix_m16n16k8<true/*is_A*/>(&q_tile[warp_id * Wr][h * 8], 
	...
	for (int j = 0; j <= i; j++)
	{
		...
		{
			auto k4 = reinterpret_cast<const float4*>(&k[(inner_row + offset) * inp_C + inner_col]);
		...
		for (int offset = 0; offset < Bc; offset += stride)
		{
			auto v4 = reinterpret_cast<const float4*>(&v[(inner_row + offset) * inp_C + inner_col]);
	...
	// Need to do unpermutation from (B, NH, T, HS) to (B, T, NH, HS)
	out += ((b * T + t + warp_id * Wr) * NH + nh) * HS;
...
) 
{
	...
	flash_attention_permute_kernel<Bc, Br, Wr, d, BLOCK_SIZE><<<grid, BLOCK_SIZE>>>(out, inp, T, NH, scale);
}

void flash_attention_permute_forward(
	float* __restrict__ out, 
...
{
	int HS = C / NH;
	flash_attention_permute(out, inp, B, T, NH, HS);
}